            ...
            @foreach (setting()->get($active_theme.'_social_links_name_'.app()->getLocale()) as $key => $social_link_name)
                @if($social_link_name || setting()->get($active_theme.'_social_links_icon_'.app()->getLocale())[$key])
                    <div class="row gutters-5">
                        <div class="col-4">
                            <div class="form-group">
                                <input type="text" class="form-control" placeholder="https://" name="{{$active_theme}}_social_links_name[]" value="{{setting()->get($active_theme.'_social_links_name_'.app()->getLocale())[$key]}}">
                            </div>
                        </div>
                        {{-- <div class="col-3">
                            <div class="form-group">
                        ...
                        <div class="col-3 button-add-icon">
                            ...
                            <input type="hidden" name="{{$active_theme}}_social_links_icon[]" id="MyIconInput-{{$key}}" value="{{setting()->get($active_theme.'_social_links_icon_'.app()->getLocale())[$key]}}">
                        </div>
                        <div class="col-3">
